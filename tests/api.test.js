...
test('POST /api/report/weekly — fails gracefully on missing profile', async () => {
  ...
  global.fetch = async (url, options) => {
    // Let local server calls pass through
    ...
    return {
      ...
      status: 200,
      json: async () => ({
        candidates: [{ content: { parts: [{ text: 'Mocked weekly report' }] } }]
      })
    };
  };
...
test('GET /api/mistakes/review-challenge — returns challenge or message when empty', async () => {
  const originalFetch = global.fetch;
  global.fetch = async (url, options) => {
    if (url.toString().startsWith(baseUrl)) return originalFetch(url, options);
    return {
      ok: true,
      status: 200,
      json: async () => ({
        candidates: [{ content: { parts: [{ text: 'Mocked challenge generated by LLM' }] } }]
      })
    };
  };
